...
class FConv(nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embed = nn.Embedding(20, self.input_encoding_size)
        ...
        self.att_transA = nn.Sequential(nn.Linear(self.rnn_size, self.rnn_size),
                                    nn.ReLU(),
                                    nn.Dropout(self.drop_prob_lm))
        self.att_transB = nn.Sequential(nn.Linear(self.rnn_size, self.rnn_size),
                                    nn.ReLU(),
                                    nn.Dropout(self.drop_prob_lm))
        self.alpha = nn.Linear(self.rnn_size, 1)
        self.beta = nn.Linear(self.rnn_size, 1)
    ...
    def core(self, fc_feats, att_feats, output_encode, xt):
        ...
        return self.attout_embed(spatial_att), output_encode
